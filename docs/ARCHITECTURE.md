...
Lodestone is built as a Clean / Onion architecture with MVVM at the presentation edge. The guiding
rule is that dependencies point inward. The domain knows nothing about WPF, HTTP, or the file
system; those are details that plug into interfaces (ports) defined further in.
...
- **Replaceability.** Modrinth vs CurseForge, JSON-on-disk vs a future database, WPF vs a future
  macOS UI: each is a swap behind an interface rather than a rewrite.
- **Honouring constraints.** "No background processes" and "changes persist until the user acts"
...
| Dependency Inversion | every `I*` port in `Application` | UI and Infra depend on abstractions. |
...
| Factory | `IModSourceRegistry`, installer factory by `ContentType` | Centralised creation and selection. |
...
| Adapter | Modrinth DTO to domain mappers | Keeps external API shapes out of the domain. |
...
| Null Object | `Loader.None` | Resource packs and shaders have "no loader" without nulls. |
...
| Mediator (light) | `IMessageBus` | Toasts and cross-VM signals without hard references. |
| Builder | `CompatibilityReportBuilder` | Assemble a per-item issue set incrementally. |

## Key flows

### Drag-and-drop install
`MainWindow.Drop` calls `InstallLocalFileUseCase`:

1. `IArchiveMetadataReader` inspects the archive in memory (no extraction) to find the mod id,
   loader, declared dependencies and content type.
2. `IContentInstaller` copies the file into `.minecraft/<mods|resourcepacks|shaderpacks>` for the
   active game version, handling duplicates softly.
3. `IInstalledContentRepository` records it, `CompatibilityService` re-scans, and the UI shows any
   issue symbol immediately.

### Browse to install (Modrinth)
`BrowseViewModel.Search` calls `IModSource.SearchAsync` (via `CachingModSource` plus retry), then
`InstallFromCatalogUseCase` resolves the best `ProjectVersion` for the active game version and
loader, `IDownloader` fetches it (bounded by the concurrent-downloads setting) and verifies sha512.

### Compatibility detection
On every refresh, `CompatibilityService` builds lookup indexes once, then runs the rule pipeline over
the installed set for the active version. Each `ICompatibilityRule` yields zero or more
...
### Updates
`RefreshUpdatesUseCase` runs on app start and on manual refresh only. For each enabled item it asks
the source for the latest version compatible with the active game version and loader. "Update all"
and opt-in auto-update both reuse this resolver. The previous file is moved to a trash folder so a
bad update is recoverable.

## Deployment and auto-update

- Packaging and self-update use Velopack. The `release.yml` workflow builds on a `v*` tag, produces
  the installer and delta packages, and publishes a GitHub Release that acts as the update feed for
  installed clients.
...
bounded by a `SemaphoreSlim` sized from settings. Lists are virtualized. Compatibility scans are O(n)
over the library using pre-built indexes.
